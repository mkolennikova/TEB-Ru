...
def plot_forcing(df, title=None, save_path=None, figsize=(14, 10)):
    """
    ...
    # 1. Pressure
    ax = axes[0]
    ax.plot(df.index, df['Forc_PS'] / 100, color='black', linewidth=1.5)
    ...
    ax = axes[1]
    ax.plot(df.index, df['Forc_DIR_SW'], label='Direct SW', color='orange', linewidth=1.5)
    ax.plot(df.index, df['Forc_SCA_SW'], label='Diffuse SW', color='gold', linewidth=1.5)
    ax.plot(df.index, df['Forc_LW'], label='LW', color='red', linewidth=1.5)
    ...
    ax.plot(df.index, df['Forc_TA'], color=color1, linewidth=1.5)
    ...
    ax2.plot(df.index, df['Forc_QA'], color=color2, linewidth=1.5)
    ...
    ax = axes[3]
    rain_positive = df['Forc_RAIN'].clip(lower=0)
    snow_positive = df['Forc_SNOW'].clip(lower=0)

    # Bar width (auto)
    if len(df) > 1:
        width = (df.index[1] - df.index[0]).total_seconds() / 3600 / 24 * 0.8
    else:
        width = 0.8

    ax.bar(df.index, rain_positive, width=width, label='Rain', color='green')
    ax.bar(df.index, snow_positive, width=width, label='Snow', color='blue',  bottom=rain_positive)
