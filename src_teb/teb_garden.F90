...
REAL, DIMENSION(:)  , INTENT(IN)    :: PRUNOFF_GR_EXT     ! greenroof surface runoff
! COSMO + TEB GARDEN
...
!
 CALL URBAN_SOLAR_ABS(TOP, T, B, DMT, ZDIR_SW, ZSCA_SW, PZENITH, PAZIM,  &
                      ...
                      T%XSVF_GARDEN, ZALB_GR, PDN_RF, ZDF_RF,       &
...
!
  ...
  CALL TEB  (icell, iblock, TOP, T, BOP, B, TIR, DMT, OGARDEN_EXT, HIMPLICIT_WIND, PBEM_AC,     &
             ...
             ZDELT_RD, ZTSRAD_GD, ZQV_GD, PLEW_RF, ZUW_GR, PLEW_RD, PLE_WL_A,              &
...
IF (TOP%LGREENROOF) THEN
  ...
  IF (OGREENROOF_EXT) THEN
    ...
    DMT%XABS_SW_GREENROOF(:) = (1.-ZALB_GR(:)) * ZREC_SW_RF
    DMT%XABS_LW_GREENROOF(:) = ZEMIS_GR * ZREC_LW_RF - XSTEFAN * ZEMIS_GR * ZTSRAD_GR**4
    ZRN_GD(:) =  DMT%XABS_SW_GREENROOF(:) + DMT%XABS_LW_GREENROOF(:)
  ENDIF
ELSE
  !
  ZRN_GR    (:) = 0.
