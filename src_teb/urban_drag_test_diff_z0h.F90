...
!     #########
    SUBROUTINE URBAN_DRAG(icell, iblock, TOP, T, B, HIMPLICIT_WIND, PTSTEP, PTIME, PT_CANYON, PQ_CANYON, &
                          ...
                          PPEW_A_COEF_LOWCAN, PPEW_B_COEF_LOWCAN, PZ0_GARDEN,      &
...
IF (.NOT. TOP%LCANOPY) THEN
  ZVMOD_TOWN = WIND_THRESHOLD(PVMOD,PZREF+ T%XBLD_HEIGHT/3.)
  ZUSTAR_TOWN = 0.4 * ZVMOD_TOWN / LOG( (PZREF+ T%XBLD_HEIGHT/3.)/T%XZ0_TOWN )
   
  ZZ0H_TOWN = T%XZ0_TOWN * 7.4 * EXP( - 1.29 *( T%XZ0_TOWN*ZUSTAR_TOWN/1.461e-5)**0.25)
  !ZZ0H_TOWN(JJ) = T%XZ0_TOWN(JJ) * 7.4 * EXP( - 2.46 *( T%XZ0_TOWN(JJ)*ZUSTAR_TOWN(JJ)/1.461e-5)**0.25)
  !ZZ0H_TOWN(JJ) = T%XZ0_TOWN(JJ) / ZZ0_O_Z0H
  
  CALL URBAN_EXCH_COEF('MASC95', ZZ0H_TOWN, ZTS_TOWN, ZQ_TOWN, PEXNS, PEXNA, PTA, PQA,     &
                       PZREF+ T%XBLD_HEIGHT/3., PUREF+T%XBLD_HEIGHT/3., PVMOD, T%XZ0_TOWN, &
                       PRI, PCD, PCDN, ZAC, ZRA, ZCH, ILMO_TOWN                              )
...
ELSE
   ZVMOD_ROOF = WIND_THRESHOLD(PVMOD,PZREF)
   ZUSTAR_ROOF = 0.4 * ZVMOD_ROOF / LOG( PZREF/ZZ0_ROOF )
   
   ZZ0H_ROOF = ZZ0_ROOF * 7.4 * EXP( - 1.29 *( ZZ0_ROOF*ZUSTAR_ROOF/1.461e-5)**0.25)
   !ZZ0H_ROOF(JJ) = ZZ0_ROOF(JJ) * 7.4 * EXP( - 2.46 *( ZZ0_ROOF(JJ)*ZUSTAR_ROOF(JJ)/1.461e-5)**0.25)
   !ZZ0H_ROOF(JJ) = ZZ0_ROOF(JJ) / ZZ0_O_Z0H

   CALL URBAN_EXCH_COEF('MASC95', ZZ0H_ROOF, PTS_ROOF, ZQ_ROOF, PEXNS, PEXNA, PTA, PQA, &
                        PZREF, PUREF, PVMOD, ZZ0_ROOF, ZRI, ZCD, ZCDN, PAC_ROOF,        &
                        ZRA_ROOF, PCH_ROOF, ILMO_ROOF               )
...
IF (.NOT. TOP%LCANOPY) THEN 
  ZZ0H_TOP = ZZ0_TOP
  CALL URBAN_EXCH_COEF('MASC95', ZZ0H_TOP, PT_CANYON, PQ_CANYON, PEXNS, PEXNA, PTA, PQA,    &
                      PZREF+T%XBLD_HEIGHT-PZ_LOWCAN, PUREF+T%XBLD_HEIGHT-PZ_LOWCAN, &
                      PVMOD, ZZ0_TOP,  ZRI, ZCD, ZCDN, PAC_TOP, ZRA_TOP, PCH_TOP, ILMO_TOP  )
...
!  ZZ0_GROUND(:) = ZZ0_ROAD(:) * T%XROAD  (:)/(T%XROAD(:)+T%XGARDEN(:)) + ZZ0_GARDEN(:) * T%XGARDEN  (:)/(T%XROAD(:)+T%XGARDEN(:))
  ...
  ZVMOD_ROAD(:) = WIND_THRESHOLD(PU_LOWCAN(:)+ZW_CAN,PZ_LOWCAN(:))
  ZUSTAR_ROAD(:) = 0.4 * ZVMOD_ROAD(:) / LOG( PZ_LOWCAN/ZZ0_ROAD(:) )
   
  ZZ0H_ROAD(:)= ZZ0_ROAD(:) * 7.4 * EXP( - 1.29 *( ZZ0_ROAD(:)*ZUSTAR_ROAD(:)/1.461e-5)**0.25)
  !ZZ0H_ROAD(JJ) = ZZ0_ROAD(JJ) * 7.4 * EXP( - 2.46 *( ZZ0_ROAD(JJ)*ZUSTAR_ROAD(JJ)/1.461e-5)**0.25)
  !ZZ0H_ROAD(JJ) = ZZ0_ROAD(JJ) / ZZ0_O_Z0H
   
  CALL URBAN_EXCH_COEF('MASC95', ZZ0H_ROAD, PTS_ROAD, PQ_LOWCAN, PEXNS, PEXNA,  &
						PT_LOWCAN, PQ_LOWCAN, PZ_LOWCAN, PZ_LOWCAN,              &
						PU_LOWCAN+ZW_CAN, ZZ0_ROAD, ZRI, ZCD_ROAD, ZCDN,         &
						PAC_ROAD, ZRA_ROAD, PCH_ROAD, ILMO_ROAD        )
...
IF (TOP%LGARDEN) THEN
   ...
   ZVMOD_GARDEN(:) = WIND_THRESHOLD(PU_LOWCAN(:),PZ_LOWCAN(:))
   ZUSTAR_GARDEN(:) = 0.4 * ZVMOD_GARDEN(:) / LOG( PZ_LOWCAN/PZ0_GARDEN(:) )
   
   ZZ0H_GARDEN(JJ)= PZ0_GARDEN(JJ) * 7.4 * EXP( - 1.29 *( PZ0_GARDEN(JJ)*ZUSTAR_GARDEN(JJ)/1.461e-5)**0.25)
   !ZZ0H_GARDEN(:) = PZ0_GARDEN(:) * 7.4 * EXP( - 2.46 *( PZ0_GARDEN(:)*ZUSTAR_GARDEN(:)/1.461e-5)**0.25)
   !ZZ0H_GARDEN(JJ) = PZ0_GARDEN(JJ) / ZZ0_O_Z0H
   
   CALL URBAN_EXCH_COEF('MASC95', ZZ0H_GARDEN, PTS_GARDEN, PQS_GARDEN, PEXNS, PEXNA,  &
                       PT_LOWCAN, PQ_LOWCAN, PZ_LOWCAN, PZ_LOWCAN,              &
                       PU_LOWCAN, PZ0_GARDEN, ZRI, PCD_GARDEN, ZCDN_GARDEN,         &
                       PAC_GARDEN, ZRA_GARDEN, PCH_GARDEN, ILMO_GARDEN        )	
   
   ZVMOD_TERRA(:) = WIND_THRESHOLD(PVMOD(:),PZREF(:))
   ZUSTAR_TERRA(:) = 0.4 * ZVMOD_TERRA(:) / LOG( PZREF(:)/PZ0_GARDEN(:) )
   
   ZZ0H_TERRA(JJ)= PZ0_GARDEN(JJ) * 7.4 * EXP( - 1.29 *( PZ0_GARDEN(JJ)*ZUSTAR_TERRA(JJ)/1.461e-5)**0.25)
   !ZZ0H_TERRA(:)= PZ0_GARDEN(:) * 7.4 * EXP( - 2.46 *( PZ0_GARDEN(:)*ZUSTAR_TERRA(:)/1.461e-5)**0.25)
   !ZZ0H_TERRA(JJ) = PZ0_GARDEN(JJ) / ZZ0_O_Z0H

   CALL URBAN_EXCH_COEF('MASC95', ZZ0H_TERRA, PTSRAD_GR, PRUNOFF_GR, PEXNS, PEXNA,  &
                       PTA, PQA, PZREF, PZREF,              &
                       PVMOD, PZ0_GARDEN, ZRI, PCD_TERRA, ZCDN_TERRA,         &
                       ZAC_TERRA, ZRA_TERRA, PCH_TERRA, ILMO_TERRA        )						   
ENDIF
