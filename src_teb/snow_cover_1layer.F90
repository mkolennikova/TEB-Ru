...
!*      2.2    snow depth
  PSNOW_D(JI) = PTSTEP * PSR(JI) / PRHOSMIN
!*      2.3    snow thermal conductivity
...
!
  ...
  ZWORK1(JI) = PSNOW_D(JI) * ZSNOW_HC(JI) / PTSTEP
!
...
!              ----------------------------------------------
!
  ZWORK1(JI) = ZSNOW_TC(JI)/(0.5*PSNOW_D(JI))
!
...
!
  ZMELT(JI)  = MIN( ZMELT(JI) , ZWSNOW(JI) / PSNOW_D(JI) / PTSTEP )
!
...
!
  ...
  PGSNOW(JI) = ZSNOW_TC(JI)/(0.5*PSNOW_D(JI)) * ( TPSNOW%T(JI,1) - PTG_COEFB(JI) ) &
             / ( 1. + ZSNOW_TC(JI)/(0.5*PSNOW_D(JI))*PTG_COEFA(JI) )
!
!
...
!
WHERE (GSNOWMASK(:))
  ZPEI_SNOW(:) = ZSNOW_HC(:)*PSNOW_D(:)*TPSNOW%T(:,1)
ELSEWHERE
  ZPEI_SNOW(:) = 0.
END WHERE
