...
!     #########
    SUBROUTINE TEB_GARDEN (icell, iblock, TOP, T, BOP, B, TPN, TIR, DMT,                                         &
                           ...
                           PPEW_B_COEF_LOWCAN, PZ0_GARDEN, PPS, PPA, PEXNS, PEXNA, PTA, PQA, PRHOA, PCO2,     &
						   ...
						   PALB_GR, PEMIS_GR, PTSRAD_GR, PH_GR, PLE_GR, PEVAP_GR, PRUNOFF_GR,     &
						   PALB_GD, PEMIS_GD, PTSRAD_GD, PQV_GD, PH_GD, PLE_GD, PEVAP_GD,         &
						   PCH_GD, PCD_GD, PRUNOFF_GD, PCH_RD, PCH_RF, PCH_WL, PCH_TOP, PAC_TOP,  &
...
REAL, DIMENSION(:)  , INTENT(IN)    :: PZ0_GARDEN         ! garden roughness length
...
REAL, DIMENSION(:)  , INTENT(INOUT) :: PTSRAD_GR          ! greenroof radiative surface temp. (snow free) 
REAL, DIMENSION(:)  , INTENT(INOUT) :: PH_GR              ! sensible heat flux over greenroofs 
REAL, DIMENSION(:)  , INTENT(INOUT) :: PLE_GR             ! latent heat flux over greenroofs 
REAL, DIMENSION(:)  , INTENT(INOUT) :: PEVAP_GR           ! total evaporation over greenroofs (kg/m2/s)
REAL, DIMENSION(:)  , INTENT(INOUT) :: PRUNOFF_GR         ! greenroof surface runoff
! COSMO + TEB GARDEN
REAL, DIMENSION(:)  , INTENT(IN)    :: PALB_GD            ! garden albedo
REAL, DIMENSION(:)  , INTENT(IN)    :: PEMIS_GD           ! garden emissivity 
REAL, DIMENSION(:)  , INTENT(INOUT) :: PTSRAD_GD          ! garden radiative surface temp. (snow free) 
REAL, DIMENSION(:)  , INTENT(INOUT) :: PQV_GD             ! garden specific humidity
REAL, DIMENSION(:)  , INTENT(INOUT) :: PH_GD              ! sensible heat flux over garden 
REAL, DIMENSION(:)  , INTENT(INOUT) :: PLE_GD             ! latent heat flux over garden 
REAL, DIMENSION(:)  , INTENT(INOUT) :: PEVAP_GD           ! total evaporation over garden (kg/m2/s)
...
REAL, DIMENSION(:)  , INTENT(INOUT) :: PRUNOFF_GD         ! garden surface runoff
...
! MT
REAL, DIMENSION(SIZE(PTA))  :: WSNOW_ROOF_CHECK    ! new snow content to exclude very low snow rate
REAL, DIMENSION(SIZE(PTA))  :: WSNOW_ROAD_CHECK    ! new snow content to exclude very low snow rate
...
!REAL, DIMENSIOn(SIZE(PTA)) :: ZH_GR, ZLE_GR, ZRUNOFF_GR
REAL, DIMENSION(SIZE(PTA)) :: ZDRAIN_GR, ZIRRIG_GR
!REAL, DIMENSION(SIZE(PTA)) :: ZEVAP_GR, ZTSRAD_GR
!
...
!
 CALL URBAN_SOLAR_ABS(TOP, T, B, DMT, ZDIR_SW, ZSCA_SW, PZENITH, PAZIM,  &
                      TPN%XFRAC_PANEL, TPN%XALB_PANEL, PALB_GD,     &
                      T%XSVF_GARDEN, PALB_GR, PDN_RF, ZDF_RF,       &
                      PDN_RD, ZDF_RD, ZREC_SW_RD, ZREC_SW_SN_RD,    &
                      ZREC_SW_WL_A, ZREC_SW_WL_B, ZREC_SW_GD,       &
                      ZREC_SW_RF, PDIR_ALB_TWN, PSCA_ALB_TWN,       &
                      PSO_ALB_TWN, ZSW_UP_SO, ZSW_RAD_GD, ZREC_SW_WIN,         &
					  PREF_SW_GRND, PREF_SW_FAC, ZE_SHADING, B%LSHAD_DAY, GSHADE )
...
!              -------------
!
  CALL URBAN_LW_COEF(B, T, PLW_RAD, PEMIS_GD, T%TSNOW_ROAD%TS, PTSRAD_GD,  &
...
!
  ...
  CALL TEB  (icell, iblock, TOP, T, BOP, B, TIR, DMT, HIMPLICIT_WIND, PBEM_AC,     &
             ...
             PPEW_B_COEF_LOWCAN, PZ0_GARDEN, PPS, PPA, PEXNS, PEXNA, PTA, PQA, PRHOA, PLW_RAD, &
             PRR, PSR, PZREF, PUREF, PVMOD, PH_TRAFFIC, PLE_TRAFFIC, PTSTEP,       &
             ZDF_RF, PDN_RF, ZDF_RD, PDN_RD, ZQSAT_RF, ZQSAT_RD, ZDELT_RF,         &
             ZDELT_RD, PTSRAD_GD, PQV_GD, PLEW_RF, ZUW_GR, PLEW_RD, PLE_WL_A,              &
             PLE_WL_B, PRNSN_RF, PHSN_RF, PLESN_RF, PGSN_RF, PMELT_RF,             &
             ZRN_GR, PH_GR, PLE_GR, ZGFLUX_GR, ZDRAIN_GR, PTSRAD_GR, PRUNOFF_GR,              &
             ...
             PT_RAD_IND, PHU_BLD, PTIME, ZE_SHADING, PMELT_BLT, PSNOWD_RF,         &
			 PSNOWD_RD, PCH_GD, PCD_GD, PCH_RD, PCH_RF, PCH_WL, PCH_TOP, ILMO_ROAD,&
             ILMO_ROOF, ILMO_TOP, PCD_TERRA, PCH_TERRA			 )
...
!              -----------
!
 CALL AVG_URBAN_FLUXES(icell, iblock, TOP, T, B, TPN, DMT,                                               &
                       PTS_TWN, PEMIS_TWN, PT_CAN, PQ_CAN, PT_LOWCAN, PQ_LOWCAN,          &
                       ZTA, ZQA, PRHOA, PPS, PH_TRAFFIC,  PLE_TRAFFIC, ZWL_O_GRND,        &
                       ZESN_RF, PEMIS_GR, PLW_RAD,  PAC_RF, ZAC_RF_WAT, PAC_WL, PAC_RD,  &
                       ...
                       PTSRAD_GD, ZRN_GD, PH_GD, PLE_GD, ZGFLUX_GD, PEVAP_GD,             &
                       PRUNOFF_GD, PEVAP_GR, PRUNOFF_GR, ZDRAIN_GR,                       &
                       PRN_GRND, PH_GRND, PLE_GRND, PGFLX_GRND, PRN_TWN, PH_TWN, PLE_TWN, &
                       PGFLX_TWN, PEVAP_TWN, ZEMIT_LW_RD,ZEMIT_LW_GD, PEMIT_LW_GRND, PEMIS_GD, PLW_UP )
!
...
IF (TOP%LGARDEN) THEN
  ...
  ZREC_LW_GD(:) = (ZLW_S_TO_G  (:) * (ZT_SKY(:)       - PTSRAD_GD(:))   &
                 + ZLW_WA_TO_G (:) * (T%XT_WALL_A(:,1)- PTSRAD_GD(:))   &
                 + ZLW_WB_TO_G (:) * (T%XT_WALL_B(:,1)- PTSRAD_GD(:))   &
                 + ZLW_WIN_TO_G(:) * (B%XT_WIN1(:)    - PTSRAD_GD(:)) ) &
...
!
  ...
  CALL GARDEN(HIMPLICIT_WIND, TOP%TTIME, PTSUN, PPEW_A_COEF_LOWCAN, PPEW_B_COEF_LOWCAN, &
              ...
              ZREC_SW_GD, ZREC_LW_GD, PU_LOWCAN,  PQV_GD, ZRN_GD, PH_GD, PLE_GD, ZGFLUX_GD,     &
              ZSFCO2_GD, PEVAP_GD, ZUW_GD, PRUNOFF_GD, PAC_GD, ZQSAT_GD, PTSRAD_GD,     &
              ZAC_AGG_GD, ZHU_AGG_GD, ZDRAIN_GD, ZIRRIG_GD )

  PAC_GD_WAT(:) = PAC_GD(:)
  DMT%XABS_SW_GARDEN(:) = (1.-PALB_GD(:)) * ZREC_SW_GD
  DMT%XABS_LW_GARDEN(:) = PEMIS_GD(:) * ZREC_LW_GD(:) - XSTEFAN * PEMIS_GD(:) * PTSRAD_GD(:)**4 
  ZEMIT_LW_GD(:) = XSTEFAN * PTSRAD_GD(:)**4 + (1 - PEMIS_GD(:)) / PEMIS_GD(:) * DMT%XABS_LW_GARDEN(:)
  ! COSMO+TEB GARDEN
...
IF (TOP%LGREENROOF) THEN
  ...
  CALL GREENROOF(HIMPLICIT_WIND, TOP%TTIME, PTSUN, PPEW_A_COEF, PPEW_B_COEF,         &
                 ...
                 ZREC_SW_RF, ZREC_LW_RF, PVMOD, PALB_GR, ZRN_GR, PH_GR, PLE_GR,               &
                 ZGFLUX_GR, ZSFCO2_GR, PEVAP_GR, ZUW_GR,                             &
                 PAC_GR, ZQSAT_GR, PTSRAD_GR, ZAC_AGG_GR, ZHU_AGG_GR,                &
                 DMT%XG_GREENROOF_ROOF, PRUNOFF_GR, ZDRAIN_GR, ZIRRIG_GR ) 
  !
  PAC_GR_WAT(:) = PAC_GR(:)
  DMT%XABS_SW_GREENROOF(:) = (1.-PALB_GR(:)) * ZREC_SW_RF
  DMT%XABS_LW_GREENROOF(:) = PEMIS_GR * ZREC_LW_RF - XSTEFAN * PEMIS_GR * PTSRAD_GR**4
